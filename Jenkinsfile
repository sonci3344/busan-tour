pipeline {
    ...
    stages {
        stage('Git Clone') {
            steps {
                echo 'Git Clone'
                git url: 'https://github.com/sonci3344/busan-tour.git',
        ...
        stage('Docker Image Build') {
            steps {
                ...
                dir("${env.WORKSPACE}") {
                    sh """
                       docker build -t sonci3344/busan-tour:$BUILD_NUMBER .
                       docker tag sonci3344/busan-tour:$BUILD_NUMBER sonci3344/busan-tour:latest
                    """
                }
            }
        }
        ...
        stage('Docker Image Push') {
            steps {
                ...
                sh "docker push sonci3344/busan-tour:latest"  // docker push
            }
        }
        stage('Cleaning up') { 
              steps { 
              ...
              sh """
                  docker rmi sonci3344/busan-tour:$BUILD_NUMBER
                  docker rmi sonci3344/busan-tour:latest
              """
            }
        }
        stage('Upload S3') { 
